...
namespace Core
{
    ...
    {
        [Header("Debug")]
        public bool OverrideGizmoToggle;
        ...
        private void OnDrawGizmos()
        {
            ...
            if (OverrideGizmoToggle)
            {
                enabled = GizmosEnabledOverride;
            }
            ...
            for (int y = 0; y < rows; y++)
            {
                for (int x = 0; x < cols; x++)
                {
                    ...
                    if (cell != null)
                    {
                        var localPos = Board.transform.InverseTransformPoint(cell.transform.position);
                    ...
                    if (cell != null && cell.BusyTicks > 0)
                    {
                        c = Color.Lerp(c, Color.yellow, 0.6f);
                    }
                    ...
                    var labelPos = center + Vector3.up * 0.2f;
